...
function parseArgs(argv) { const values = {}; for (let index = 0; index < argv.length; index += 2) { const flag = argv[index]; const value = argv[index + 1]; if (!value || values[flag] || !["--repository-root", "--phase"].includes(flag)) throw new Error(`invalid scaffold freeze option: ${String(flag)}`); values[flag] = value; } if (!isAbsolute(values["--repository-root"]) || !["tag", "evidence"].includes(values["--phase"])) throw new Error("absolute repository root and phase are required"); return Object.freeze({ repositoryRoot: resolve(values["--repository-root"]), phase: values["--phase"] }); }
...
export async function buildScaffoldLock(root, baselineCommit, briefBytes, policyBytes) {
...
async function commitEvidence(root) {
  ...
  const status = command("git", ["status", "--porcelain=v1", "--untracked-files=all"], { cwd: root }).stdout.toString("utf8").trim().split("\n").filter(Boolean).sort(); assert.deepEqual(status, ["?? conformance/poiesis-v1/birth-brief.md", "?? conformance/poiesis-v1/birth-workspace-policy.json"]);
  const [briefBytes, policyBytes] = await Promise.all([readFile(briefPath), readFile(policyPath)]); const lock = await buildScaffoldLock(root, baselineCommit, briefBytes, policyBytes); await writeFile(lockPath, `${JSON.stringify(lock, null, 2)}\n`, { flag: "wx", mode: 0o644 });
...
export async function freezeScaffold(options) { return options.phase === "tag" ? tagScaffold(options.repositoryRoot) : commitEvidence(options.repositoryRoot); }
