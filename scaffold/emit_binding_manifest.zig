...
const application = @import("poiesis_application");
...
fn writeInterface(
    ...
    try out.print(
        ...
        .{ operation, site_identity, interface_label, &interface_id, &payload_schema_id, &result_schema_id, effect.authority_requirements, maximum_result_bytes, if (trailing) "," else "" },
    );
}
...
pub fn main(init: std.process.Init) !void {
    ...
    try out.print(
        ...
        .{ &application_id, App.Manifest.application_name, App.Manifest.application_version, &contract_digest },
    );
    try writeInterface(out, "decide", Compiled.DecisionSite.semantic_identity, "model.decide.v1", @intCast(Compiled.Definition.decision.maximum_result_bytes), true);
    ...
    try writeInterface(out, "search", Compiled.ActionSites[3].semantic_identity, "repo.release-search.v1", schemaMaximum(Compiled.ActionSites[3]), true);
    try writeInterface(out, "check", Compiled.ActionSites[4].semantic_identity, "repo.check.v1", schemaMaximum(Compiled.ActionSites[4]), true);
